...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  const [mode, setModeState] = useState<ThemeMode>(() => {
    return (localStorage.getItem("kliq_ctrl_theme") as ThemeMode) ?? "dark";
  });
  const [effective, setEffective] = useState<"dark" | "light">(() => getEffective(
    (localStorage.getItem("kliq_ctrl_theme") as ThemeMode) ?? "dark"
  ));
  ...
  const setMode = (m: ThemeMode) => {
    localStorage.setItem("kliq_ctrl_theme", m);
    setModeState(m);
